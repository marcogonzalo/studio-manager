...
- **Local:** Tras `supabase start`, `pnpm run seed:demo` suele bastar (lee `API_URL` y `SERVICE_ROLE_KEY` de `supabase status`; no mezcla una variable de entorno con otra del status: o defines **las dos** `SUPABASE_URL` y `SUPABASE_SERVICE_ROLE_KEY`, o ninguna). A mano: mismas claves en el JSON de `supabase status`.
