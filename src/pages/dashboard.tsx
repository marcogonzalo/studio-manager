...
export default function Dashboard() {
  ...
  const fetchDashboardStats = async () => {
    try {
      ...
      // Proyectos activos al final del mes pasado: proyectos que fueron completados después del final del mes pasado o que aún no están completados
      const { data: activeProjectsLastMonth, error: activeLastMonthError } = await supabase
        .from('projects')
        .select('id, completed_date')
        .or(`completed_date.is.null,completed_date.gt.${lastDayLastMonth.toISOString()}`);
      ...
      const additionalCostsTotal = additionalCosts?.reduce((sum, cost) => sum + Number(cost.amount || 0), 0) || 0;
      ...
      if (activeError || activeLastMonthError || clientsError || newClientsError || additionalCostsError || confirmedOrdersError || recentError) {
      ...
      const activeChange = activeLastMonthCount > 0 
        ? Math.round(((activeCount - activeLastMonthCount) / activeLastMonthCount) * 100)
        : activeCount > 0 ? 100 : 0;
      ...
      setStats({
        ...
        totalIncomeThisMonth: 0, // Placeholder para futuro control de pagos
            ...
            {loading ? (
              ...
              <>
                <div className="text-2xl font-bold text-foreground">
                  {new Intl.NumberFormat('es-ES', { style: 'currency', currency: 'EUR' }).format(stats.totalExpensesThisMonth)}
                </div>
                <p className="text-xs text-muted-foreground mt-1">
                  Este mes
                </p>
        ...
        <Card className="border-none shadow-md bg-gradient-to-br from-white to-secondary/20 hover:shadow-lg transition-all duration-300 group">
          ...
          <CardContent>
            {loading ? (
              ...
              <>
                <div className="text-2xl font-bold text-foreground">
                  {new Intl.NumberFormat('es-ES', { style: 'currency', currency: 'EUR' }).format(stats.totalIncomeThisMonth)}
                </div>
                <p className="text-xs text-muted-foreground mt-1">
      ...
      <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-7">
        <Card className="col-span-4 border-none shadow-md">
          <CardHeader>
            ...
            <CardDescription>
              {loading ? 'Cargando...' : stats.recentProjects.length > 0 
                        ...
                        <div className="flex items-center gap-4 mt-2 text-xs text-muted-foreground">
                          {project.client?.full_name && (
                            <div className="flex items-center gap-1">
                              ...
                              <span className="truncate">{project.client.full_name}</span>
                            </div>
                          )}
                          {project.start_date && (
                            <div className="flex items-center gap-1">
