...
function AuthContent() {
  ...
  // Keep document.title in sync when user toggles login/signup without navigation
  useEffect(() => {
    let title = "";
    if (isLogin) {
      ...
      const planLabel =
        planFromUrl === "STUDIO"
          ...
          : planFromUrl === "PRO"
            ? "Pro"
            : "Base";
      title = `Crea tu cuenta con el plan ${planLabel}`;
    } else {
...
export function AuthContentWithSuspense() {
  return (
    <Suspense
      fallback={
        <div className="bg-background flex min-h-screen items-center justify-center p-4">
