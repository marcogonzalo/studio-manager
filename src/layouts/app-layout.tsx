...
import { LayoutDashboard, FolderKanban, ShoppingBag, Truck, Settings, LogOut, Menu, Users, Leaf, PanelLeftClose, PanelLeft } from 'lucide-react';
...
export default function AppLayout() {
  ...
  const SidebarContent = ({ collapsed = false }: { collapsed?: boolean }) => (
    <div className="flex flex-col h-full bg-sidebar border-r border-border relative">
      <div className={cn("p-6 flex items-center gap-2", collapsed && "justify-center p-4")}>
          ...
          const linkContent = (
            ...
            >
              <item.icon className={cn(
                ...
                !collapsed && "mr-3",
                location.pathname === item.href ? "text-primary-foreground" : "text-muted-foreground group-hover:text-secondary-foreground"
              )} />
              {!collapsed && item.name}
            </Link>
          );
      ...
      <div className="flex min-h-screen bg-background text-foreground">
        ...
        <Sheet open={isMobileOpen} onOpenChange={setIsMobileOpen}>
          <SheetTrigger asChild>
            <Button variant="ghost" size="icon" className="md:hidden fixed bottom-4 left-1/2 -translate-x-1/2 z-50 print:hidden bg-background/80 backdrop-blur-sm border border-border shadow-sm">
