...
export function ProjectPurchases({ projectId }: { projectId: string }) {
  ...
  const fetchOrders = async () => {
    ...
    const { data, error } = await supabase
      ...
      .select('*, supplier:suppliers(name), project_items(id, name, quantity, unit_cost)')
  ...
  const handleDelete = async (orderId: string) => {
    if (!confirm('¿Está seguro de eliminar esta orden de compra? Los ítems asociados volverán a estado pendiente, a menos que estén en otra orden activa.')) {
      return;
    }
  ...
  const calculateOrderTotal = (items: { quantity: number; unit_cost: number }[]) => {
                  ...
                  <div className="flex justify-between items-start">
                    <div className="flex-1">
                      <div className="flex items-center gap-3 mb-2">
                        <CardTitle className="text-lg">{po.supplier?.name || 'Proveedor Desconocido'}</CardTitle>
                        <span className={`px-2 py-1 rounded-full text-xs font-medium ${STATUS_COLORS[po.status] || STATUS_COLORS.draft}`}>
