...
- [x] **Assets table and storage refactor:** Tabla `assets` (id, user_id, source, url, storage_path, bytes, mime_type, kind, owner_table, owner_id). Dominio: `project_documents`, `space_images`, `products` con `asset_id` (FK opcional). Al subir producto/documento/space image se crea fila en `assets`, se devuelve `assetId` y se guarda `asset_id`; al borrar o reemplazar se eliminan assets y archivos en B2. Triggers: `user_storage_usage` desde `assets` y legacy (cuando `asset_id` es null); borrado en cascada de assets al eliminar producto/documento/space_image. Lib `src/lib/assets.ts` (`createAsset`, `deleteAssetById`, `getAssetIdByOwner`). Tests en `assets.test.ts` y aserciones de `assetId` en rutas de upload.
...
- [x] **Settings y cambio de planes (fixes #20):** Vista Configuración (`/settings` → `/settings/account`) con sidebar reducida (Volver, Dashboard, Cuenta, Tu plan). Cuenta: card Email y Zona de peligro (eliminar cuenta). Tu plan: card con plan actual (nombre, periodicidad si hay renovación), consumibles (proyectos, clientes, proveedores, productos) con barras de progreso y límites desde `effectivePlan.config`, histórico de asignaciones vencidas. Modifica tu plan (`/settings/plan/change`): planes PRO y STUDIO, switch Mensual/Anual, adquirir plan vía RPC `assign_plan`, refetch del plan y redirección. Perfil separado de Cuenta: ruta `/profile` (Perfil, solo nombre y empresa); `/settings/account` (Cuenta: correo y zona de peligro). Ruta `/account` eliminada; enlaces "Mi perfil" apuntan a `/profile`. Botón flotante menú móvil: tamaño +25%, color secondary.
...
- [x] **Audit veta.pro (squirrelscan / fix/audit-veta-website):** Mejoras SEO, accesibilidad y contenido tras auditoría. **Auth:** H1, `<main id="main-content">`, skip link y enlace "Volver al inicio"; título dinámico por URL con `generateMetadata` (Iniciar sesión / Regístrate / Plan); página servidor con `auth-content.tsx` cliente; eliminado layout auth redundante. **Logo:** `alt="Veta"` en `VetaLogo`. **Meta:** Títulos ≤50 caracteres (template "Veta - %s"); descripciones más largas en home, pricing, about, contact, legal; og:image/twitter en layout marketing. **Skip link:** Clase `.skip-link` en globals.css; añadido en marketing layout, app layout y auth; logo del sidebar ya no hace de skip (solo `/dashboard`). **Sitemap:** Comentario aclaratorio en `sitemap.ts` (solo `/sitemap.xml`; errores de auditoría por URLs probadas por convención). **Contraste:** `--muted-foreground` ajustado en light y dark. **Docs:** `docs/audit-website-notes.md` (sitemap, LinkedIn 999, sugerencias keyword density). **Otros:** Título home sin duplicar "Veta" (uso de template); logo sidebar sin `#main-content`.
