...
- [x] **Marketing Fase 1 (SEO y conversión):** Menú móvil con Sheet en marketing layout; formulario de contacto funcional (Resend); email coherente con Veta (`hola@veta.app`); `sitemap.ts` y `robots.ts`; metadata y Open Graph por página; imagen OG dinámica; Twitter Cards; `metadataBase` en root layout. Ver `docs/marketing-seo-analysis.md`.
