...
- [x] **Usage Plans (Planes de uso):** Sistema de planes Prueba (BASE), Pro (PRO) y Studio (STUDIO). Migraciones: tablas `plans` (consumibles y modalidades: budget_mode, costs_management, documents, etc.) y `plan_assignments`, función `get_effective_plan`. AuthProvider expone `effectivePlan`. Vista Pricing actualizada; registro con plan en URL; pill de plan en Mi cuenta y menú. Restricciones por plan: moneda/impuesto por proyecto, pestañas Compras/Pagos/Documentos, opciones PDF y "Excluir del proyecto" (budget_mode), Importe real y coste interno (costs_management), renders (documents). Control de costes accesible para todos con overlay deshabilitado en plan base.
